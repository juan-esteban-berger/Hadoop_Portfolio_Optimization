...
public class CovJoinWeights extends Configured implements Tool {
    ...
    public int run(String[] args) throws Exception {
        if (args.length != 3) {
            System.err.println("Usage: CovJoinWeights <mean input path> <return input path> <output path>");
            return -1;
        }

        Job job = Job.getInstance(getConf(), "Mean Join");
        job.setJarByClass(CovJoinWeights.class);
    ...
    public static void main(String[] args) throws Exception {
        int exitCode = ToolRunner.run(new CovJoinWeights(), args);
